...
UNICONV_EXPORT UniconvResult* uniconv_plugin_execute(const UniconvRequest* request) {
    ...
    if (opts.fps < 1) opts.fps = 1;
    if (opts.fps > 30) opts.fps = 30;
    ...
    if (request->dry_run) {
        ...
        if (opts.width > 0) extra << ", \"width\": " << opts.width;
        if (opts.start > 0) extra << ", \"start\": " << opts.start;
        if (opts.duration > 0) extra << ", \"duration\": " << opts.duration;
